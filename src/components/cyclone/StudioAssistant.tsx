...
const QUICK = [
  ...
  "What is the bottleneck / highest idleness?",
  "What was steady-state productivity?",
  "Suggest increasing trucks by 2",
];
  ...
  const [messages, setMessages] = useState<UiMsg[]>([
    {
      ...
      role: "system",
      text:
        "AI Assistant is bound to your current Format Prompt, CYCLONE network, and last simulation results. Ask about productivity, idleness, or request prompt edits (e.g. set trucks to 8). Proposed prompts are never applied until you click Apply.",
    },
  ]);
  ...
  async function send(text?: string) {
    ...
    try {
      ...
      if (res.source === "local" && res.error) {
        toast.message("Assistant local mode", { description: res.error });
      }
    } catch (e) {
  ...
  function applyPrompt(next: string) {
    ...
    setMessages((m) => [
      ...m,
      {
        id: uid("s"),
        role: "system",
        text: "Format Prompt applied to the studio. Use Draw Model to refresh the network, then Simulate.",
      },
    ]);
  }
