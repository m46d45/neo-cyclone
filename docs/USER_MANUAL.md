...
The studio footer can show **how many times** Draw Model and Simulate have been used. Counts are **anonymous** (no prompt text, no model). Worldwide totals are stored in a durable counter (or in your own Neon database if `DATABASE_URL` is set). You also see **this device** counts in the browser. Increments are rate-limited so a single IP cannot inflate the number.
